...
void palAopqkBatch ( double *rap, double *dap, const int nStars,
  ...
  double *cachedZDT = malloc(nStars * sizeof(double));
  double *cachedZDOBS = malloc(nstars * sizeof(double));
  ...
  int iStar,iZdobs,iMin;
  double minZdobs,ddZdobs;

  for(iStar = 0; iStar < nStars; iStar++){
    /*  apparent ra,dec to cartesian -ha,dec */
    palDcs2c( rap[iStar]-st, dap[iStar], v );
    xhd = v[0];
    yhd = v[1];
    zhd = v[2];
     ...
     *  ---------- */
    minZdobs=-1.0;
    for(iZdobs = 0; iZdobs<iStar; iZdobs++){
        if(cachedZDT[iZdobs]<zdt)ddZdobs = zdt-cachedZDT[iZdobs];
        else ddZdobs = cachedZDT[iZdobs]-zdt;

        if (ddZdobs<minZdobs || minZdobs<0.0){
            minZdobs = ddZdobs;
            iMin = iZdobs;
        }
    }

    if(minZdobs>=0.0 && minZdobs<0.001){
        zdobs = cachedZDOBS[iMin];
    }
    else{
